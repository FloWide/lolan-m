...
typedef struct {
  ...
  uint8_t notfound;
  bool toodeep;
  uint8_t invalid_keys;
} lolan_BunchUpdateOutputStruct;


extern bool isPathValid(const uint8_t *path);
extern uint8_t pathDefinitionLevel(lolan_ctx *ctx, const uint8_t *path, uint8_t *occurrences, bool occ_maxrec);
extern uint8_t lolanVarFlagCount(lolan_ctx *ctx, uint16_t flags, bool *dlbpsame, uint8_t *defLevel, uint8_t *bpath);
...
extern int8_t lolanGetDataFromCbor(CborValue *it, uint8_t *data, uint8_t data_max, uint8_t *data_len, uint8_t *type);
...
extern int8_t lolanVarDataToCbor(uint8_t *data, uint8_t data_len, lolan_VarType type, CborEncoder *encoder);
extern int8_t lolanVarToCbor(lolan_ctx *ctx, const uint8_t *path, uint8_t index, CborEncoder *encoder);
